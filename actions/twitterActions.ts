...
export class TwitterActions {
  ...
  constructor(page: Page) {
    this.page = page;
    this.generalActions = new GeneralActions(this.page);
  }
  ...
  async postToTwitter(caption: string, imageUrl?: string) {
    await this.page.goto("https://x.com");
      ...
      await this.generalActions.wait(3);
    ...
    try {
      ...
      if (options?.text) {
        ...
        if (options.imageUrl) {
          ...
          const imageBuffer = await this.generalActions.downloadImage(options.imageUrl);
          await this.generalActions.uploadImage(imageBuffer);
        }
